...
static void spi_trans_done_cb(spi_transaction_t *trans)
{
    (void)trans;
    if (s_spi_sem) {
...
{
    ESP_LOGI(TAG, "Initializing SPI for ST77916 with DMA");
    ...
    esp_err_t ret = spi_bus_initialize(ST77916_SPI_HOST, &bus_config, SPI_DMA_CH_AUTO);
    if (ret == ESP_ERR_INVALID_STATE) {
        ESP_LOGW(TAG, "SPI bus already initialized, reinitializing");
        spi_bus_free(ST77916_SPI_HOST);
        ret = spi_bus_initialize(ST77916_SPI_HOST, &bus_config, SPI_DMA_CH_AUTO);
    }
    if (ret != ESP_OK) {
    ...
    ESP_LOGI(TAG, "SPI initialized with DMA: SCLK=%d, MOSI=%d, CS=%d, Freq=%dHz",
...
esp_err_t st77916_write_cmd(uint8_t lcd_cmd, const uint8_t *param, size_t param_size)
{
    ...
    /* 必须使用内部 DMA 内存，避免 PSRAM 在 flash 操作时引发 cache 错误 */
    uint8_t *tx_buf = heap_caps_malloc(total_size, MALLOC_CAP_DMA | MALLOC_CAP_INTERNAL);
...
esp_err_t st77916_write_color_dma(const uint8_t *data, size_t data_len)
{
    if (s_spi_dev == NULL) return ESP_ERR_INVALID_STATE;
    if (!data || data_len == 0) return ESP_OK;
    ...
    return spi_device_queue_trans(s_spi_dev, &t, portMAX_DELAY);
}

esp_err_t st77916_wait_dma_done(void)
{
    if (!s_spi_sem) return ESP_ERR_INVALID_STATE;
    spi_transaction_t *rtrans;
    return spi_device_get_trans_result(s_spi_dev, &rtrans, portMAX_DELAY);
}
